...
#ifndef UNTITLED2_UNROLLED_LINKED_LIST_H
#define UNTITLED2_UNROLLED_LINKED_LIST_H
...
struct element{
    ...
    friend bool operator>=(const element &x,const element &y){return (x>y||x==y);}
    friend bool operator!=(const element &x,const element &y){return !(x==y);}
...
public:
    ...
    UnrolledLinkedList(std::string name1,std::string name2):element_count(0),file_name_all(name1),file_name_lead(name2){
        ...
        index_.write(reinterpret_cast<char *>(&min_element), sizeof(element<_key_type, _subkey_type, _value_type>));
        index_.seekp(sizeof(block<_key_type, _subkey_type, _value_type>),std::ios::beg);
        index_.write(reinterpret_cast<char *>(&max_element), sizeof(element<_key_type, _subkey_type, _value_type>));
        index_.close();
        return;
    }
...
template<typename _key_type, typename _subkey_type, typename _value_type>
void UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Insert(const _key_type& key, const _subkey_type& subkey, const _value_type& value){
    lead_search.clear();
    lead_search.open(file_name_lead);
    if(!lead_search){
        lead_search.open(file_name_lead,std::ofstream::out);
    ...
    for(int i=0;i<lead_count;++i){//search the lead
        ...
        if(cmp==0){lead_search.close();index_.close();throw 1;}
            ...
            if (i == 0 && cmp < 0) {
                ...
                ++j;
                for (int k = block_size - 1; k >= j; --k) test_block.element_array[k + 1] = test_block.element_array[k];
                ...
                if (test_lead.block_size < 960) {
                    ...
                    const long long int new_block_loc = max_count * sizeof(block<_key_type, _subkey_type, _value_type>);
                    block<_key_type, _subkey_type, _value_type> new_block;
                    for (int k = mid; k < test_lead.block_size; k++) new_block.element_array[k - mid] = test_block.element_array[k];
                    index_.clear();
                    index_.seekp(new_block_loc, std::ios::beg);
                    index_.write(reinterpret_cast<char *>(&new_block), sizeof(block<_key_type, _subkey_type, _value_type>));
                    ...
                    const long long int new_lead_loc =
                            2 * sizeof(int) + sizeof(long long int) + max_count * sizeof(lead<_key_type, _subkey_type, _value_type>);
                    lead_search.seekp(new_lead_loc, std::ios::beg);
                    lead_search.write(reinterpret_cast<char *>(&new_lead), sizeof(lead<_key_type, _subkey_type, _value_type>));
                    lead_search.seekg(test_lead.next, std::ios::beg);
                    lead_search.read(reinterpret_cast<char *>(&new_lead), sizeof(lead<_key_type, _subkey_type, _value_type>));
...
template<typename _key_type, typename _subkey_type, typename _value_type>
void UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Modify(const _key_type& key, const _subkey_type& subkey, const _value_type& value){
    lead_search.clear();
    lead_search.open(file_name_lead);
    if(!lead_search){
        lead_search.open(file_name_lead,std::ofstream::out);
...
template<typename _key_type, typename _subkey_type, typename _value_type>
void UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Erase(const _key_type& key, const _subkey_type& subkey){
...
template<typename _key_type, typename _subkey_type, typename _value_type>
void UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Clear(){

}

template<typename _key_type, typename _subkey_type, typename _value_type>
bool UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Exist(const _key_type& key, const _subkey_type& subkey){
    lead_search.clear();
    lead_search.open(file_name_lead);
    if(!lead_search){
        lead_search.open(file_name_lead,std::ofstream::out);
...
template<typename _key_type, typename _subkey_type, typename _value_type>
std::vector<_value_type>* UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Traverse(){
    std::vector<_value_type>* v_all=new _value_type;
    v_all->reserve(element_count+2);
    lead_search.clear();
    lead_search.open(file_name_lead);
    if(!lead_search){
        lead_search.open(file_name_lead,std::ofstream::out);
    ...
    for(int i=0;i<lead_count;++i){
        ...
        index_.read(reinterpret_cast<char*>(&test_block),sizeof(block<_key_type, _subkey_type, _value_type>));
        if(i==0||i==lead_count-1) for(int j=1;j<block_size;++j) v_all->push_back(test_block.element_array[j].value);
        else for(int j=0;j<block_size;++j) v_all->push_back(test_block.element_array[j].value);
        lead_loc=test_lead.next;
    }
    return v_all;
}
...
template<typename _key_type, typename _subkey_type, typename _value_type>
std::vector<_value_type>* UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Traverse(const _key_type& key){
    std::vector<_value_type>* v_all=new _value_type;
    v_all->reserve(element_count+2);
    lead_search.clear();
    lead_search.open(file_name_lead);
    ...
    for(int i=0;i<lead_count;++i){
        ...
        if(test_lead.this_element.key<=key) {
            ...
            for (int j = 0; j < block_size; ++j) {
                if (test_block.element_array[j].key == key) v_all->push_back(test_block.element_array[j].value);
            }
            loc = test_lead.next;
            continue;
        }
...
template<typename _key_type, typename _subkey_type, typename _value_type>
[[nodiscard]] _value_type UnrolledLinkedList<_key_type, _subkey_type, _value_type>::Get(const _key_type& key, const _subkey_type& subkey){
    lead_search.clear();
    lead_search.open(file_name_lead);
    if(!lead_search){
        lead_search.open(file_name_lead,std::ofstream::out);
...
#endif //UNTITLED2_UNROLLED_LINKED_LIST_H
